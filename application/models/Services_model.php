...
class Services_model extends CI_Model {
	...
	public function get_all($params = FALSE){
		...
		if(!isset($params['all'])){
			$this->db->where('ser_show',1);
			$this->db->group_start();
				$this->db->or_where('ser_deleted_at','');
				$this->db->or_where('ser_deleted_at IS NULL',null,true);
				$this->db->or_where('ser_deleted_at','0000-00-00 00:00:00');
			$this->db->group_end();
		}
