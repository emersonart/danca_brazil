...
class Contacts_model extends CI_Model {
	...
	protected $table_type = 'contacts_type';
	...
	public function get_all($params = FALSE){
		$this->db->join($this->table_type, $this->table_type.".cot_id = ".$this->table.".con_cot_id",'inner');
		...
		if(!isset($params['not_deleted']) || $params['not_deleted'] == false){
			$this->db->group_start()->where('con_deleted_at IS NULL',null,true)->or_where('con_deleted_at',"")->or_where('con_deleted_at','0000-00-00 00:00:00')->group_end();
		}
     ...
     public function delete($con_id)
    {
        ...
        $params = [
        	'con_deleted_at' => date('Y-m-d H:i:s'),
        ];
        return $this->db->update($this->table,$params);
    }
    ...
    {
        $this->db->delete($this->table,array('con_id'=>$tes_id));
    }
